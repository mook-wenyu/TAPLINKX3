...
1. `Task 15` 已完成前台相机可行性验证：前台取流可用、短时生命周期稳定、手可进入画面。
2. 下一步收紧为 `Task 16: single-gesture foreground recognizer spike`，只验证一个最小手势语义能否在当前前台流里稳定成立。
3. 将目标 App 审计继续作为并行证据轨道，而不是重新变回视觉主线的硬门槛。
4. 即使前台相机已证明可用，仍然不扩张到后台常驻宣称、多手势词汇、悬浮窗主开关或白名单深适配实现。
